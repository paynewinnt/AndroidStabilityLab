...
QUICK_ADB_COMMANDS: tuple[QuickAdbCommand, ...] = (
    ...
    QuickAdbCommand(
        ...
        "framework",
        "日志与证据",
        ("shell", "sh", "-c", "ls -lt /data/anr 2>/dev/null | head -n 30"),
    ...
    QuickAdbCommand(
        ...
        "framework",
        "日志与证据",
        ("shell", "sh", "-c", "dumpsys dropbox --print | tail -n 240"),
        timeout_seconds=25,
    ),
    QuickAdbCommand(
    ...
    QuickAdbCommand(
        "system_cpuinfo",
        "CPU 使用摘要",
        "查看系统按进程聚合的 CPU 使用情况，辅助 CPU 高负载与卡顿定位。",
        "system_server",
        "资源与调度",
    ...
    QuickAdbCommand(
        ...
        "native_service",
        "Native 证据",
        ("shell", "sh", "-c", "ls -lt /data/tombstones 2>/dev/null | head -n 30"),
        risk="may_require_privilege",
    ),
    QuickAdbCommand(
        "hal_binder_state",
        "Binder state",
        "查看 binder 调用、线程和等待状态；不可读时会直接提示权限或挂载原因。",
        "hal",
        "Binder / HAL 调用",
        (
            "shell",
    ...
    QuickAdbCommand(
        ...
        "kernel",
        "存储与 I/O",
        ("shell", "sh", "-c", "cat /proc/pressure/io 2>/dev/null"),
        risk="may_require_privilege",
    ),
    QuickAdbCommand(
        "kernel_diskstats",
    ...
    QuickAdbCommand(
        ...
        "kernel",
        "温度与频率",
        ("shell", "sh", "-c", "for z in /sys/class/thermal/thermal_zone*/temp; do echo $z=$(cat $z 2>/dev/null); done"),
        timeout_seconds=15,
        risk="may_require_privilege",
    ),
    QuickAdbCommand(
        ...
        "kernel",
        "温度与频率",
        ("shell", "sh", "-c", "for c in /sys/devices/system/cpu/cpu*/cpufreq/scaling_cur_freq; do echo $c=$(cat $c 2>/dev/null); done"),
        timeout_seconds=15,
        risk="may_require_privilege",
    ),
    QuickAdbCommand(
