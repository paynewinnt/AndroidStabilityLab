...
from stability.time_utils import coerce_datetime, format_beijing_datetime_or_original


MAX_PERFORMANCE_RUN_WINDOW_SECONDS = 24 * 60 * 60


class MonitoringPayloadMixin:
    ...
    def _decorate_run_detail_instances(self, instances: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in instances:
            ...
            raw_captured_at = str(dict(snapshot_payload).get("timestamp", "") or "") if snapshot_payload else ""
            captured_at = format_beijing_datetime_or_original(raw_captured_at) or "" if snapshot_payload else ""
            samples = []
            for index, sample_payload in enumerate(sample_payloads):
                raw_sample_captured_at = str(dict(sample_payload).get("timestamp", "") or "")
                ...
                samples.append(
                    {
                        ...
                        "raw_captured_at": raw_sample_captured_at,
                        "captured_at": format_beijing_datetime_or_original(raw_sample_captured_at) or "",
    ...
    def _recent_monitoring_snapshot(
        self,
        *,
        run_limit: int,
        entry_limit: int,
        max_run_window_seconds: int | None = None,
        limit_entries: bool = True,
    ) -> dict[str, Any]:
        service = getattr(self._bundle, "run_history_service", None)
                    ...
                    samples = [
                        {
                            ...
                            "raw_captured_at": str(monitoring.get("raw_captured_at", "") or ""),
                    ...
                    entries.append(
                        {
                            ...
                            "run_started_at": str(detail.get("started_at", "") or ""),
                            "run_finished_at": str(detail.get("finished_at", "") or ""),
                            ...
                            "backend": backend,
                            "raw_captured_at": str(sample_payload.get("raw_captured_at", "") or ""),
        ...
        if max_run_window_seconds is not None:
            entries = self._limit_entries_to_run_window(entries, max_seconds=max_run_window_seconds)
        entries.sort(key=lambda item: (str(item.get("captured_at", "") or ""), str(item.get("run_created_at", "") or "")), reverse=True)
        limited_entries = entries[: max(0, int(entry_limit))] if limit_entries else entries
    ...
    @classmethod
    def _limit_entries_to_run_window(cls, entries: list[dict[str, Any]], *, max_seconds: int) -> list[dict[str, Any]]:
        """Keep each run's visible samples within the first max_seconds of that run."""
        if max_seconds <= 0:
            return []
        grouped: dict[tuple[str, str], list[dict[str, Any]]] = {}
        passthrough: list[dict[str, Any]] = []
        for entry in entries:
            parsed = cls._entry_timestamp(entry)
            if parsed is None:
        ...
        limited: list[dict[str, Any]] = list(passthrough)
        for group_entries in grouped.values():
            ordered = sorted(group_entries, key=lambda item: cls._entry_timestamp(item) or datetime.min)
            first_timestamp = cls._entry_timestamp(ordered[0])
            if first_timestamp is None:
                limited.extend(ordered)
                continue
            for item in ordered:
                timestamp = cls._entry_timestamp(item)
                if timestamp is None or (timestamp - first_timestamp).total_seconds() <= max_seconds:
                    limited.append(item)
        return limited

    @staticmethod
    def _entry_timestamp(entry: Mapping[str, Any]) -> datetime | None:
        return coerce_datetime(str(entry.get("raw_captured_at", "") or entry.get("captured_at", "") or ""))
    ...
    def _monitoring_app_packages(snapshot_payload: Mapping[str, Any]) -> list[str]:
        ...
        for item in list(snapshot_payload.get("apps", []) or []):
            if not isinstance(item, Mapping):
                continue
            package_name = str(
                item.get("app_package", "")
                or item.get("package_name", "")
                or dict(item.get("app_info", {}) or {}).get("package_name", "")
                or ""
            ).strip()
    ...
    def _monitoring_metrics(snapshot_payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "system_cpu_usage": _pick(system.get("cpu_usage")),
            "memory_pss": _pick(primary_app.get("memory_pss"), system.get("memory_pss")),
            "memory_java": _pick(primary_app.get("memory_java"), system.get("memory_java")),
            "memory_native": _pick(primary_app.get("memory_native"), system.get("memory_native")),
            "memory_graphics": _pick(primary_app.get("memory_graphics"), system.get("memory_graphics")),
            "fps": _pick(primary_app.get("fps"), system.get("fps")),
            "frame_time_ms": _pick(primary_app.get("frame_time_ms"), primary_app.get("frame_time"), system.get("frame_time_ms"), system.get("frame_time")),
            "frame_time_avg": _pick(primary_app.get("frame_time_avg"), system.get("frame_time_avg")),
            "frame_time_max": _pick(primary_app.get("frame_time_max"), system.get("frame_time_max")),
            "frame_time_99p": _pick(primary_app.get("frame_time_99p"), primary_app.get("frame_time_p99"), system.get("frame_time_99p"), system.get("frame_time_p99")),
            ...
            "gpu_temperature": _pick(primary_app.get("gpu_temperature"), system.get("gpu_temperature")),
            ...
            "battery_temperature": _pick(system.get("battery_temperature")),
            "battery_voltage": _pick(system.get("battery_voltage")),
            ...
            "perfetto_trace_size_bytes": _pick(system.get("perfetto_trace_size_bytes")),
            "perfetto_duration_ms": _pick(system.get("perfetto_duration_ms")),
        }
...
__all__ = ["MonitoringPayloadMixin"]
