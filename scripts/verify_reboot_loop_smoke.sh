...
usage() {
  cat <<'EOF'
Usage:
  scripts/verify_reboot_loop_smoke.sh --package-name PACKAGE [options]

Warning:
  This is an end-to-end smoke that sends a real `adb reboot` to the target device.
  Run it only on a device that is safe to reboot.
...
OUTPUT_DIR="/tmp/android_stability_lab_reboot_loop_smoke_${TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
...
echo "WARNING: reboot_loop smoke will send a real adb reboot to the target device." >&2

select_device
ensure_device_online

TASK_NAME_SUFFIX="$(printf '%s' "${PACKAGE_NAME}" | tr '.:' '__')"
TASK_NAME="reboot_loop_smoke_${TASK_NAME_SUFFIX}_${TIMESTAMP}"
...
CREATE_TASK_ARGS=(
  ...
  --template-type reboot_loop
  --device "${DEVICE_ID}"
  --created-by smoke_script
  --timeout-seconds "${TASK_TIMEOUT_SECONDS}"
  --task-params "${TASK_PARAMS_JSON}"
  --note "reboot_loop cli smoke verification; this task intentionally reboots the target device"
)
...
echo
echo "Reboot Loop CLI smoke summary"
