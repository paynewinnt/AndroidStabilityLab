...
usage() {
  cat <<'EOF'
Usage:
  scripts/verify_foreground_background_loop_smoke.sh --package-name PACKAGE [options]
...
Optional:
  ...
  --loop-count N                      Foreground/background loop count. Default: 2.
  --foreground-wait-ms MS             Wait after foreground launch. Default: 800.
  --background-wait-ms MS             Wait after Home key. Default: 800.
  --launch-timeout-seconds SEC        am start timeout. Default: 20.
  --home-timeout-seconds SEC          Home key timeout. Default: 10.
...
LAUNCH_ACTIVITY=""
LOOP_COUNT=2
FOREGROUND_WAIT_MS=800
BACKGROUND_WAIT_MS=800
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --launch-timeout-seconds)
      ...
      shift 2
      ;;
    --home-timeout-seconds)
      HOME_TIMEOUT_SECONDS="${2:-}"
      shift 2
      ;;
...
OUTPUT_DIR="/tmp/android_stability_lab_foreground_background_smoke_${TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
...
payload = {
    ...
    "launch_timeout_seconds": int(sys.argv[4]),
    "home_timeout_seconds": int(sys.argv[5]),
}
...
TASK_NAME="foreground_background_loop_smoke_${TASK_NAME_SUFFIX}_${TIMESTAMP}"
...
CREATE_TASK_ARGS=(
  ...
  --template-type foreground_background_loop
  --device "${DEVICE_ID}"
  --created-by smoke_script
  --timeout-seconds "${TASK_TIMEOUT_SECONDS}"
  --task-params "${TASK_PARAMS_JSON}"
  --note "foreground_background_loop cli smoke verification"
)
...
echo
echo "Foreground/Background Loop CLI smoke summary"
