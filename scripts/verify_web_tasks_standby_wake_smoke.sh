...
usage() {
  cat <<'EOF'
Usage:
  scripts/verify_web_tasks_standby_wake_smoke.sh --package-name PACKAGE [options]

Warning:
  This smoke starts the local Web portal and will sleep/wake a real Android device.
...
Optional:
  ...
  --loop-count N                  Standby/wake loop count. Default: 1.
  --standby-wait-ms MS            Wait after sleep key event. Default: 300.
  --wake-wait-ms MS               Wait after wake key event. Default: 300.
  --command-timeout-seconds SEC   adb keyevent timeout. Default: 10.
  --task-timeout-seconds SEC      Task timeout metadata. Default: 60.
  --no-unlock-after-wake          Do not send the post-wake unlock key event.
...
OUTPUT_DIR="/tmp/android_stability_lab_web_tasks_standby_wake_smoke_${TIMESTAMP}"
...
payload = {
    ...
    "command_timeout_seconds": int(sys.argv[4]),
    "unlock_after_wake": sys.argv[5] == "1",
}
...
assert_html_contains "${TASKS_HTML}" "standby_wake_loop"
...
TASK_NAME="web_standby_wake_smoke_${TASK_NAME_SUFFIX}_${TIMESTAMP}"
...
post_form "${BASE_URL}/tasks/actions/create-task" "${CREATE_TASK_HTML}" \
  ...
  --data-urlencode "template_type=standby_wake_loop" \
  ...
  --data-urlencode "note=standby_wake_loop web tasks smoke"
...
echo
echo "Web /tasks standby/wake smoke summary"
