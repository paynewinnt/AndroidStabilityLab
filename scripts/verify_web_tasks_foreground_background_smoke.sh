...
usage() {
  cat <<'EOF'
Usage:
  scripts/verify_web_tasks_foreground_background_smoke.sh --package-name PACKAGE [options]
...
Optional:
  ...
  --port PORT                     Web port. Default: 8035.
  ...
  --task-timeout-seconds SEC      Task timeout metadata. Default: 90.
...
OUTPUT_DIR="/tmp/android_stability_lab_web_tasks_foreground_background_smoke_${TIMESTAMP}"
...
assert_html_contains "${TASKS_HTML}" "foreground_background_loop"
...
TASK_NAME="web_foreground_background_smoke_${TASK_NAME_SUFFIX}_${TIMESTAMP}"
...
post_form "${BASE_URL}/tasks/actions/create-task" "${CREATE_TASK_HTML}" \
  ...
  --data-urlencode "template_type=foreground_background_loop" \
  ...
  --data-urlencode "note=foreground_background_loop web tasks smoke"
...
echo
echo "Web /tasks foreground/background smoke summary"
