...
usage() {
  cat <<'EOF'
Usage:
  scripts/verify_standby_wake_loop_smoke.sh --package-name PACKAGE [options]

Warning:
  This smoke talks to a real Android device and will send sleep/wake key events.
...
OUTPUT_DIR="/tmp/android_stability_lab_standby_wake_smoke_${TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
...
TASK_NAME="standby_wake_loop_smoke_${TASK_NAME_SUFFIX}_${TIMESTAMP}"
...
echo
echo "Standby/Wake Loop CLI smoke summary"
