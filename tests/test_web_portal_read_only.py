...
class WebPortalReadOnlyPagesTest(unittest.TestCase):
    ...
    def test_home_page_shows_local_ops_console_notice_by_default(self) -> None:
        ...
        self.assertEqual(status, 200)
        self.assertIn("text/html", content_type)
        self.assertNotIn('<div class="content-topbar">', html)
        self.assertNotIn("当前 Web 入口按本地运维控制台设计", html)
        self.assertNotIn("先把平台今天发生了什么", html)
    ...
    def test_platform_page_renders_team_entry_boundary_summary(self) -> None:
        ...
        self.assertNotIn("平台聚合设备状态", html)
        self.assertNotIn("当前 Web 入口按本地运维控制台设计", html)
        self.assertNotIn("团队共享入口模式：所有查看者默认看到同一份平台数据", html)
        self.assertNotIn("平台关键服务已就绪", html)
        self.assertNotIn("仍有关键服务未就绪", html)
    ...
    def test_home_page_highlights_remote_access_warning_when_enabled(self) -> None:
        ...
        self.assertIn("Runner 摘要卡", html)
        self.assertNotIn('<div class="content-topbar">', html)
        self.assertNotIn("已显式允许远程绑定到 0.0.0.0", html)
        self.assertNotIn("缺少正式认证、授权和审计边界", html)
    ...
    def test_long_run_templates_page_and_manifest_expose_entry(self) -> None:
        ...
        self.assertNotIn("admin-page-header", html)
        self.assertNotIn("查看长稳默认值和可覆盖参数", html)
    ...
    def test_performance_page_renders_recent_monitoring_summary(self) -> None:
        ...
        self.assertIn("text/html", content_type)
        self.assertIn("性能采样", html)
        self.assertIn("href='/tasks'><strong>任务</strong>", html)
        self.assertIn("href='/runs'><strong>Run</strong>", html)
        self.assertIn("href='/artifacts'><strong>产物</strong>", html)
        ...
        self.assertNotIn('<div class="content-topbar">', html)
        self.assertNotIn("performance-help-root", html)
        self.assertNotIn("三步上手", html)
        self.assertIn("采样说明", html)
        self.assertIn("当前页面聚合最近", html)
